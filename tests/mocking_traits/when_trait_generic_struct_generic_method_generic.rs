...
mod and_method_is_static {
    ...
    fn and_not_mocked_then_runs_normally() {
        ...
        assert_eq!(
            "true 2.5 a",
            Struct::<&str>::static_method(true, 2.5f32, 'a')
        );
        assert_eq!(
            "true 2.5 abc",
            Struct::<u8>::static_method(true, 2.5f32, "abc")
        );
    }
    ...
    fn and_continue_mocked_then_runs_with_modified_args_for_mocked_type_only() {
        unsafe {
            <Struct<u8> as Trait<char>>::static_method::<f32>
                .mock_raw(|a, b, c| MockResult::Continue((!a, b + 1., c.to_ascii_uppercase())));
        }

        assert_eq!(
            "false 3.5 A",
            Struct::<u8>::static_method(true, 2.5f32, 'a')
        );
        assert_eq!("true abc a", Struct::<u8>::static_method(true, "abc", 'a'));
        assert_eq!(
            "true 2.5 a",
...
mod and_method_is_ref_method {
    ...
    fn and_not_mocked_then_runs_normally() {
        ...
        assert_eq!(
            "abc true 1.5 a",
            Struct("abc").ref_method(true, 1.5f32, 'a')
        );
        assert_eq!(
            "2 true 1.5 abc",
            Struct(2u8).ref_method(true, 1.5f32, "abc")
        );
    }
...
mod and_method_is_val_method {
    ...
    fn and_not_mocked_then_runs_normally() {
        ...
        assert_eq!(
            "abc true 1.5 a",
            Struct("abc").val_method(true, 1.5f32, 'a')
        );
        assert_eq!(
            "2 true 1.5 abc",
            Struct(2u8).val_method(true, 1.5f32, "abc")
        );
    }
    ...
    fn and_return_mocked_then_returns_mocking_result() {
        ...
        assert_eq!(
            "mocked 2 true 1.5 a",
            Struct(2u8).val_method(true, 1.5f32, 'a')
        );
        assert_eq!("2 true abc a", Struct(2u8).val_method(true, "abc", 'a'));
        assert_eq!(
            "abc true 1.5 a",
